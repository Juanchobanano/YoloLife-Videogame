index = argument0;
if(ds_list_find_value(productos_comprados,index) == false){
    if(monedas >= ds_list_find_value(valor_productos,index)){
        if(src_desbloquear(index)){
...
}else{ // Si el producto ya esta comprado.
    ds_list_replace(productos_seleccionados,0,index);
}
